...
class db(object):
    ...
    def update_resource(self, resource):
        ...
        try:
            ...
            if not self.cursor.fetchone():
                ...
                self.cursor.execute(
                    "INSERT INTO resources (hostname,site) VALUES ('%s','%s')" % (resource.hostname, resource.site))
                self.cursor.execute(
                    "INSERT INTO monitor (hostname,status,timestamp) VALUES ('%s','new',current_timestamp - interval '15 minutes')" % (
                        resource.hostname))
    ...
    def status_resource(self, hostname, status):
        try:
            self.cursor.execute(
                "INSERT INTO monitor (hostname,status,timestamp) VALUES ('%s','%s',current_timestamp)" % (
                    hostname, status))
        except Exception as e:
